...
class OmniSearch
{
    ...
    public function getTickets(string $searchTerm, bool $searchInDescription, bool $searchInTimeregistrations, bool $searchInComments): array
    {
        ...
        WHERE ' . $userIdWhere . 'ticket.type IN ("task", "subtask", "bug") AND (ticket.id LIKE CONCAT("%", :searchTerm, "%") OR ticket.tags LIKE CONCAT("%", :searchTerm, "%") OR ticket.headline LIKE CONCAT("%", :searchTerm, "%")' . $whereTerm . ')
